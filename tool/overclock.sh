...
if [ "$DRIVER" == "AMD" ]; then
    ...
    for ID in $CARDS; do
      ...
      MEMSTATE=$(/root/mineros/tool/ohgodatool -i $ID --show-mem | grep -o -P "Memory state [0-9]" | grep -o "[[:digit:]]" | tail -n 1)
      ...
      [ -n "$parameters" ] && sudo /root/mineros/tool/ohgodatool -i $ID $parameters
  ...
  sudo xinit /root/mineros/tool/overclock_nvidia.sh $CoreArray $MemArray $POWER -- :0 -once -config /etc/X11/xorg.conf &
