...
if [ "$DRIVER" == "AMD" ]; then
	...
	while [ $i -le $GPU_COUNT ]; do
	    if [ -e "/sys/class/drm/card$i/device/pp_table" ]; then
	    	...
	    	STATS=`echo "$STATS" | jq ".temp += \"$(sudo /root/mineros/tool/ohgodatool --show-temp -i $i | tr -d 'C');\""`
	    	STATS=`echo "$STATS" | jq ".fan += \"$(sudo /root/mineros/tool/ohgodatool --show-fanspeed -i $i | tr -d '%');\""`
