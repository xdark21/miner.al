...
function boot()
{
	...
	sudo rm -rf /home/mineros/.cache/sessions/*
...
{
	CONFIG=$(sudo cat /home/mineros/config.json)
...
function getConfig()
{
	...
	if [ "$STATUS" == "ok" ]; then
	    echo "$GETCONFIG" | sudo tee /home/mineros/config.json > /dev/null 2>&1
	    sync
			updateconfig
	    return 0
	else
...
function backup_miner()
{
	...
	sudo cp -Rp /root/mineros/miners /var/tmp/miner/
	sleep 0.5
...
function logo()
{
	sudo cat /root/mineros/lib/logo.sh
}
# Hardware Status
function get_hardware()
{
	sudo /root/mineros/cron/hardware_status.sh > /dev/null 2>&1 &
}
# Enable WOL
function enable_wol()
{
